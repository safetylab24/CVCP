...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        reg_valid_mask = (
            batch_roi_ious > self.roi_sampler_cfg['REG_FG_THRESH']).long()

        # classification label
        if self.roi_sampler_cfg['CLS_SCORE_TYPE'] == 'cls':
            batch_cls_labels = (
                batch_roi_ious > self.roi_sampler_cfg['CLS_FG_THRESH']).long()
    ...
    def sample_rois_for_rcnn(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            while k > 0 and cur_gt[k].sum() == 0:
                k -= 1
            cur_gt = cur_gt[:k + 1]
            cur_gt = cur_gt.new_zeros((1, cur_gt.shape[1])) if len(
                cur_gt) == 0 else cur_gt
    ...
    def subsample_rois(self, max_overlaps):
        ...
        if fg_num_rois > 0 and bg_num_rois > 0:
            ...
            rand_num = torch.from_numpy(np.random.permutation(
                fg_num_rois)).type_as(max_overlaps).long()
            ...
            rand_num = np.floor(np.random.rand(
                self.roi_sampler_cfg['ROI_PER_IMAGE']) * fg_num_rois)
    ...
    def sample_bg_inds(hard_bg_inds, easy_bg_inds, bg_rois_per_this_image, hard_bg_ratio):
        if hard_bg_inds.numel() > 0 and easy_bg_inds.numel() > 0:
            ...
            rand_idx = torch.randint(
                low=0, high=hard_bg_inds.numel(), size=(hard_bg_rois_num,)).long()
            hard_bg_inds = hard_bg_inds[rand_idx]

            # sampling easy bg
            rand_idx = torch.randint(
                low=0, high=easy_bg_inds.numel(), size=(easy_bg_rois_num,)).long()
