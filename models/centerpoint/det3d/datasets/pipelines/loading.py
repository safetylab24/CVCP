...
def read_file(path, tries=2, num_point_feature=4, virtual=False):
    if virtual:
        ...
        points = np.concatenate(
            [points, virtual_points1, virtual_points2], axis=0).astype(np.float32)
    else:
        points = np.fromfile(
            path, dtype=np.float32).reshape(-1, 5)[:, :num_point_feature]
...
def read_single_waymo_sweep(sweep):
    ...
    points_sweep = np.concatenate(
        [points_xyz, points_feature], axis=-1).T  # 5 x N
    ...
    def __call__(self, res, info):
        ...
        if self.type == "NuScenesDataset":
            ...
            for i in np.random.choice(len(info["sweeps"]), nsweeps - 1, replace=False):
                sweep = info["sweeps"][i]
                points_sweep, times_sweep = read_sweep(
                    sweep, virtual=res["virtual"])
                sweep_points_list.append(points_sweep)
                sweep_times_list.append(times_sweep)

            points = np.concatenate(sweep_points_list, axis=0)
            times = np.concatenate(
                sweep_times_list, axis=0).astype(points.dtype)
