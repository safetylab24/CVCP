...
def voxelization(points, pc_range, voxel_size):
    ...
    points = points[keep, :]
    coords = ((points[:, [2, 1, 0]] - pc_range[[2, 1, 0]]) /
              voxel_size[[2, 1, 0]]).to(torch.int64)
...
def voxelization_virtual(points, pc_range, voxel_size):
    ...
    padded_points[len(real_points):len(real_points) +
                  len(painted_point), 5:19] = painted_point[:, :-2]
    padded_points[len(real_points):len(real_points) +
                  len(painted_point), 19] = painted_point[:, -1]
    ...
    coords = ((points_xyz[:, [2, 1, 0]] - pc_range[[2, 1, 0]]
               ) / voxel_size[[2, 1, 0]]).to(torch.int64)
    unique_coords, inverse_indices = coords.unique(return_inverse=True, dim=0)
    ...
    voxels[mix_mask, :5] = voxels[mix_mask, :5] / \
        indicator[mix_mask].unsqueeze(-1)
    voxels[mix_mask, 5:] = voxels[mix_mask, 5:] / \
...
class DynamicVoxelEncoder(nn.Module):
    ...
    def forward(self, points):
        ...
        for res in points:
            if self.virtual:
                voxel, coor = voxelization_virtual(res, self.pc_range.to(
                    res.device), self.voxel_size.to(res.device))
            else:
                voxel, coor = voxelization(res, self.pc_range.to(
                    res.device), self.voxel_size.to(res.device))
            voxels.append(voxel)
            coors.append(coor)
