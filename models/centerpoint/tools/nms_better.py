...
def parse_args():
    ...
    parser.add_argument(
        "--output_path", help="the path to save ensemble output")
...
def reorganize_boxes(box_lidar_nusc):
    ...
    gt_boxes_lidar = np.concatenate(
        [centers.reshape(-1, 3), wlhs.reshape(-1, 3), rots[..., np.newaxis].reshape(-1, 1)], axis=1)
...
LARGE_CLASS = ['trailer']
ALL_CLASS = ['car', 'truck', 'bus', 'construction_vehicle', 'bicycle',
             'pedestrian', 'barrier', 'traffic_cone', 'motorcycle', 'trailer']
